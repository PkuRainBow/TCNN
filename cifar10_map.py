...
def load_data():
    ...
    feature_lmdb_env = lmdb.open('D:\\users\\v-yuhyua\\fromGPU02\\feature\\SGD_iter40000_data_lmdb')
    ...
    datum = caffe_pb2.Datum()
    num = 59000
    data_feature_vector = np.zeros((num, 10), dtype=np.int16)
    for ix,(key, value) in enumerate(lmdb_cursor):
        datum.ParseFromString(value)
        data = caffe.io.datum_to_array(datum)
        data = np.squeeze(data)[:]
        data_feature_vector[ix, :] = binarize(data)
